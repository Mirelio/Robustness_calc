...
def calc_true_robustness(lims, d, cov, variance, distrib, post):
    ...
    def calculate_vol(covar, confid):
        Chi_sq_tab = {'0.10': 6.251, '0.05': 7.815, '0.025': 9.348, '0.01': 11.345, '0.005': 12.838}
        V = ((2.0 * power(math.pi, (3.0 / 2.0))) / (3.0 * ss.gamma(3.0 / 2.0))) * (power(Chi_sq_tab[confid], (3.0 / 2.0))) * sqrt(linalg.det(covar))
        return V

    prior_vol = 1/float((power((lims[0][1]) - (lims[0][0]), 3)))
    if distrib == 'normal':
        true_vol_sph = calculate_vol(cov, '0.025')
        true_robustness_sph = true_vol_sph * prior_vol
        return true_robustness_sph

    elif distrib == 'uniform':
        true_vol = float(power((lims[0][1] - d/2) - (lims[0][0] + d/2), 3))
        true_robustness = true_vol * prior_vol
        return true_robustness

# def calc_true_robustness(lims, d, cov, variance, distrib, post):
#     prior_vol = 1/float((power((lims[0][1]) - (lims[0][0]), 3)))
#     if distrib == 'normal':
#         vol_sq = power((2 * math.pi), 1.5) * sqrt(linalg.det(cov))
#         low = array([0, 0, 0])
#         upp = array([100, 100, 100])
#         mu = array([50, 50, 50])
#         p, i = mvn.mvnun(low, upp, mu, cov)
#         true_vol_sq = vol_sq*p
#
#         radius = sqrt(variance)*3
#         #radius = (max(post[0])-min(post[0]))/2
#         true_vol_sph = (4.0/3.0)*math.pi*(power(radius, 3))
...
posts = []
distrib = 'normal' # 'normal' or 'uniform'
...
for j in range(models):
    ...
    t_r_sph = calc_true_robustness(lims[j], d[j], covar[j], var[j], distrib, post)
    true_robustness.append(t_r_sph)
    posts.append(post)
...
sns.set_style("ticks")
pp = PdfPages("robustness_normal_calc_true_dsph_new.pdf")
fig, ax = plt.subplots(1,1)

for j in range(len(mean_dsphT)):
    ax.errorbar(rand_jitter(var), mean_dsphT[j], yerr=sd_d_sph.T[j], fmt='o', color=colors2[j], alpha=0.8,ecolor=colors2[j], label='%s' % (cf[j]/100.0))
#for j in range(len(mean_dsphT)):
#    ax.errorbar(rand_jitter(var), mean_dsphT[j], yerr=sd_d_sph.T[j], fmt='o', color=colors2[j],alpha=0.8, ecolor=colors2[j], label='%s' % (cf[j]/100.0))
#true_rob = ax.scatter(fract, diff, marker='o', color='#32995F')
l = ax.axhline(y=1,color='black',ls='dashed')
ax.set_xlabel('Posterior variance')
#ax.set_yscale('log')
#plt.tick_params(axis='y', which='minor')
#ax.yaxis.set_minor_formatter(FormatStrFormatter("%.1f"))
#ax.yaxis.set_major_formatter(FormatStrFormatter("%.1f"))
ax.set_xticks([256, 200, 150, 100, 50, 20])
ax.set_xticklabels(["256","200", "150", "100", "50", "20"])
